...
export default (props: IPreviewerProps) => {
  ...
  useEffect(() => {
    ...
    const handler = debounce(() => {
      ...
      const refElmTop = (ref?.current?.getBoundingClientRect().top ?? 0) + scrollTop;
      // do not offset front if the previous element is same the previewer
      const edgeOffset =
        ref.current?.previousElementSibling?.className === ref.current?.className ? navHeight : navHeight + 128;
      ...
      if (
        ...
        (scrollTop + edgeOffset > refElmTop && scrollTop + navHeight < refElmTop + (ref?.current?.offsetHeight ?? 0))
      ) {
        activeSelf();
      } else {
        setIsActive(false);
  ...
  useEffect(() => {
    const handler = (ev) => {
      if (ev.data.type === ACTIVE_MSG_TYPE && isActive && JSON.parse(ev.data.value).identifier !== props.identifier) {
        setIsActive(false);
      }
    };
  ...
  return (
    <div
      className={meta.mobile !== false ? '__dumi-default-mobile-previewer' : undefined}
      onClick={() => {
        if (isInactive) {
          activeSelf();
        }
    ...
    >
      {previewerProps && (
        <Previewer className={isActive ? '__dumi-default-previewer-target' : null} {...previewerProps} />
      )}
    </div>
  );
};
